...
def get_word_timings(transcript, audio_dur):
    """
    Create approximate word timings based on audio duration.
    """
...
def calculate_mask_interval(original_transcript, target_transcript, audio_dur, edit_type, left_margin=0.08, right_margin=0.08):
    """
    Calculate the mask interval for editing based on the edit type and transcripts.
    """
    codec_sr = 50  # Sample rate of the codec (50 Hz)
    
    # Get word boundaries
    word_bounds = get_word_timings(original_transcript, audio_dur)
    
    # Determine edit spans
    orig_span, new_span = get_span(original_transcript, target_transcript, edit_type)
    
    # Handle edge cases
    if orig_span[0] > orig_span[1]:
        raise RuntimeError(f"Invalid span for edit")
    ...
    return torch.LongTensor(mask_interval), morphed_span  # [M,2]
...
def main():
    ...
    parser.add_argument("--transcript_path", type=str, default=None, help="Path to transcript file (alternative to --original_transcript)")
    ...
    try:
        # Load original transcript from file if provided
        if args.transcript_path and not args.original_transcript:
            with open(args.transcript_path, 'r') as f:
                args.original_transcript = f.read().strip()
        ...
        mask_interval, morphed_span = calculate_mask_interval(
        ...
        print(f"Original transcript: {args.original_transcript}")
        print(f"Target transcript: {args.target_transcript}")
        print(f"Edit span (seconds): {morphed_span[0]:.2f} - {morphed_span[1]:.2f}")
