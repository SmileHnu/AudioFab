...
from .system_messages import SYSTEM_MESSAGE1, SYSTEM_MESSAGE2, SYSTEM_MESSAGE3, SYSTEM_MESSAGE4
...
class ChatSession:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # tools_description = "\n".join([tool.format_for_llm() for tool in all_tools]) , "list_available_tools"
            ...
            # system_message = SYSTEM_MESSAGE.replace("{tools_description}", tools_description)
            # system_message = SYSTEM_MESSAGE.format(tools_description= tools_description)
            # 使用SYSTEM_MESSAGE4 无需传入tools_description
    ...
    def _extract_tool_calls(self, llm_response: str) -> List[Dict[str, Any]]:
        ...
        ########################################################React################################
        # # ----------------- Added: Parse ReAct style ("tool: xxx" / "arguments: {...}") -----------------
        # # This pattern captures lines like:
        # # tool: retrieve_tools
        # # arguments: { "query": "something" }
        react_pattern = re.compile(
            r"tool\s*:\s*([\w_\-]+)[\s\S]*?arguments\s*:\s*({[\s\S]*?})",
            re.IGNORECASE,
        )

        for match in react_pattern.finditer(llm_response):
    ...
    async def send_message(
        ...
        # Append the instruction to the user message
        ...
        while tool_iteration < max_iterations:
            ...
            # Add context about task continuity
            self.messages.append({
                "role": "system",
                "content": (
                    "Please continue to complete the current task flow. If there are any more tools to be called, please continue to output JSON format tool call."
                    "If all sub-tasks are completed, please output FINAL_RESPONSE and summarize the results of all steps."
                    "Please ensure task continuity, do not interrupt or repeat completed steps."
                )
            })
